            ...
            <div class="layui-card">
                <div class="layui-card-body ">
                    <div class="layui-input-inline layui-show-xs-block">
                            ...
                            <#list onThirdMenus as onThirdMenu>
                                <#if onThirdMenu.parentId == currentMenu.id>
                                    <a onclick="${onThirdMenu.url!""}">
                                        ...
                                        <#elseif onThirdMenu.url == "deleteProductCategory();">
                        ...
                        &nbsp;&nbsp;
                        <div class="layui-input-inline layui-show-xs-block">
                            <input class="layui-input" placeholder="商品种类名称" id="search-name" value="${content!""}">
                        </div>
                        <button class="layui-btn" onclick="search();"><i class="icon iconfont">&#xe6ac;</i>&nbsp;搜索</button>
                    </div>

                    <hr>
                    <blockquote class="layui-elem-quote">Tips：商品种类名称是有字数限制的，请合理填写输入！</blockquote>
                </div>
                <div class="layui-card-body ">
                    <table class="layui-table layui-form">
                        <thead>
                        <th width="5" style="text-align:center"></th>
                        <th width="5" style="text-align:center;">商品种类ID</th>
                        <th width="500" style="text-align:center">商品种类名称</th>
                        <th width="200" style="text-align:center">商品种类创建时间</th>
                        <th width="200" style="text-align:center">商品种类更新时间</th>
                        </thead>
                        <tbody class="x-cate">
                            <#if PageInfo.list?size gt 0>
                                <#list PageInfo.list as productCategory>
                                    <tr>
                                        <td style="text-align:center">
                                            <input type="checkbox" name="" id="${productCategory.id!""}" method="checkedId" lay-skin="primary">
                                        </td>
                                        <td style="text-align:center">${productCategory.id!""}</td>
                                        <td style="text-align:center">${productCategory.categoryName!""}</td>
                                        <td style="text-align:center">${productCategory.createTime?string('yyyy-MM-dd HH:mm:ss')!""}</td>
                                        <td style="text-align:center">${productCategory.updateTime?string('yyyy-MM-dd HH:mm:ss')!""}</td>
                                    </tr>
                                </#list>
                            </#if>
                        </tbody>
                ...
                <div class="layui-card-body ">
                    <div class="page">
                        <div>
                            ...
                            <#else>
                                <a class="prev" href="/admin/product_category/index?id=${currentMenu.id!""}&pageNum=${PageInfo.prePage!""}&content=${content!""}">&lt;&lt;</a>
                            </#if>
                            <#list PageInfo.pageNum-2..PageInfo.pageNum-1 as i>
                                <#if i gte 1>
                                    <a class="num" href="/admin/product_category/index?id=${currentMenu.id!""}&pageNum=${i!""}&content=${content!""}">${i!""}</a>
                                </#if>
                            </#list>
                            ...
                            <#else>
                                <a class="next" href="/admin/product_category/index?id=${currentMenu.id!""}&pageNum=${PageInfo.nextPage!""}&content=${content!""}">&gt;&gt;</a>
                            </#if>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function search(){
        ...
        window.location.href = "/admin/product_category/index?content="+content+"&id="+${currentMenu.id};
    }
    ...
    function openEdit(){
        ...
        xadmin.open('商品种类编辑','/admin/product_category/edit?id='+id,550,200);
    }
    ...
    function deleteProductCategory(){
        ...
        layer.confirm('确认要删除<font color="red">商品种类id='+id+'</font>的这条数据吗？',function(index){
            $.ajax({
                ...
                error:function(){
                    layer.alert('网络错误！商品种类删除失败！', {icon: 5});
                }
            });
        });
    }
