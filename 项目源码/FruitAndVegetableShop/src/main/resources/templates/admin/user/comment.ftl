            ...
            <div class="layui-card">
                <div class="layui-card-body ">
                    <div class="layui-input-inline layui-show-xs-block">
                            ...
                            <#list onThirdMenus as onThirdMenu>
                                <#if onThirdMenu.parentId == currentMenu.id>
                                    <a onclick="${onThirdMenu.url!""}">
                                        ...
                                        <#elseif onThirdMenu.url == "deleteComment();">
                        ...
                        &nbsp;&nbsp;
                        <div class="layui-input-inline layui-show-xs-block">
                            <input class="layui-input" placeholder="评论内容" id="search-name" value="${content!""}">
                        </div>
                        <button class="layui-btn" onclick="search();"><i class="icon iconfont">&#xe6ac;</i>&nbsp;搜索</button>
                    </div>

                    <hr>
                    <blockquote class="layui-elem-quote">Tips：请合理规范地管理用户评论信息哦！</blockquote>
                </div>
                <div class="layui-card-body ">
                    <table class="layui-table layui-form">
                        <thead>
                        <th width="5" style="text-align:center"></th>
                        <th width="5" style="text-align:center">评论ID</th>
                        <th width="70" style="text-align:center">商品图片</th>
                        <th width="100" style="text-align:center">商品名称</th>
                        <th width="100" style="text-align:center">用户名称</th>
                        <th style="text-align:center">评论内容</th>
                        <th width="170" style="text-align:center">评论时间</th>
                        </thead>
                        <tbody class="x-cate">
                                ...
                                <#list PageInfo.list as comment>
                                    <tr>
                                        <td style="text-align:center">
                                            <input type="checkbox" name="" id="${comment.id!""}" method="checkedId" lay-skin="primary">
                                        </td>
                                        <td style="text-align:center">${comment.id!""}</td>
                                        <td style="text-align:center">
                                            <img src="/photo/view?filename=${comment.product.productPic!""}" style="width:50px;height:50px;" />
                                        </td>
                                        <td style="text-align:center">${comment.product.productName!""}</td>
                                        <td style="text-align:center">${comment.user.username!""}</td>
                                        <td style="text-align:center">${comment.content!""}</td>
                                        <td style="text-align:center">${comment.createTime?string('yyyy-MM-dd HH:mm:ss')!""}</td>
                                    </tr>
                                </#list>
                            </#if>
                        </tbody>
                ...
                <div class="layui-card-body ">
                    <div class="page">
                        <div>
                            ...
                            <#else>
                                <a class="prev" href="/admin/user/comment?id=${currentMenu.id!""}&pageNum=${PageInfo.prePage!""}&content=${content!""}">&lt;&lt;</a>
                            </#if>
                            <#list PageInfo.pageNum-2..PageInfo.pageNum-1 as i>
                                <#if i gte 1>
                                    <a class="num" href="/admin/user/comment?id=${currentMenu.id!""}&pageNum=${i!""}&content=${content!""}">${i!""}</a>
                                </#if>
                            </#list>
                            ...
                            <#else>
                                <a class="next" href="/admin/user/comment?id=${currentMenu.id!""}&pageNum=${PageInfo.nextPage!""}&content=${content!""}">&gt;&gt;</a>
                            </#if>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function search(){
        ...
        window.location.href = "/admin/user/comment?content="+content+"&id="+${currentMenu.id};
    }
    ...
    function deleteComment(){
        ...
        layer.confirm('确认要删除<font color="red">评论id='+id+'</font>的这条数据吗？',function(index){
            $.ajax({
